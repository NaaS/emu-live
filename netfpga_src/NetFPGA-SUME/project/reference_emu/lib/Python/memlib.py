...
def binaryResponseTemplate(opcode, opaque, status="00"):
	...
	response = "0000000000010000" + magic + opcode + key_length_bin +        \
...
def binaryGetRequest(kv_pair, opaque="01234567"):
	...
	request = "0000000000010000" + magic + opcode + key_length_bin +        \
...
def textSetRequest(kv_pair):
	txt_req = "set %s %d %d %d\r\n%s\r\n" % (
		...
		len(kv_pair['value']),
		kv_pair['value'])
	return "0000000000010000"+txt_req.encode('hex')

def textSetResponse(kv_pair):
	return "0000000000010000"+"STORED\r\n".encode('hex')
...
def textGetResponse(kv_pair):
	txt_res = "VALUE %s %d %d\r\n%s\r\nEND\r\n" % (
		...
		len(kv_pair['value']),
		kv_pair['value'])
	return "0000000000010000"+txt_res.encode('hex')

def textFailedGetResponse():
	return "0000000000010000"+"END\r\n".encode('hex')
...
def textDeleteResponse(kv_pair):
	return "0000000000010000"+"DELETED\r\n".encode('hex')

def textFailedDeleteResponse(kv_pair):
	return "0000000000010000"+"NOT_FOUND\r\n".encode('hex')
