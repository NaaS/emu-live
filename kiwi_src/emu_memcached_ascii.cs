...
class Emu
{
	...
	// Constants variables
	const uint BUF_SIZE = (uint)40;
	const uint MEM_SIZE = (uint)40;
...
Kiwi.Pause();
        ...
        static public uint Memcached_SET()
	{
		...
		tuser_low[0] = (dst_port<<24) | (src_port<<16) | (ulong)58;
		tkeep[7] = (byte)0x03;
		return 7U;
	}
	...
	{	
		...
		switch(cnt)
		{
			...
			case 0U:
				...
				// set the destination port in the metadata as the src port
