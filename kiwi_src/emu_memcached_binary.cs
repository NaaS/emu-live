...
class Emu
{
        ...
        static public uint Memcached_GET(bool err)
	{
		...
		if ( err ){
			...
			tuser_low[0] = (dst_port<<24) | (src_port<<16) | (ulong)82;
			tkeep[10] = (byte)0x03;
		}
		else{
			...
			tuser_low[0] = (dst_port<<24) | (src_port<<16) | (ulong)86;
			tkeep[10] = (byte)0x3f;
		}
		return 10U;
	}
        ...
        static public uint Memcached_DELETE(bool err)
	{
		...
		if( !err ){
			...
			tuser_low[0] = (dst_port<<24) | (src_port<<16) | (ulong)74;
			tkeep[9] = (byte)0x03;
			//pkt_size = 8;
			return 9U;
		}
	...
	static ulong calc_IP_checksum()
	{
		...
		ulong sum0=0x00, sum1=0x00, sum=0x00, sum_all = 0x00;

		for(i=1; i<5; i++)
		{
			data = (i==1) ? tdata[1]>>48 : (i==4) ? tdata[4]<<48 : (i==2) ? tdata[2] : (i==3) ? tdata[3] : (ulong)0x00;
			// extract every 16-bit from the stream for addition and reorder it to big endianess
			tmp0 = ((ulong)(data>>0) & (ulong)0x00ff)<<8 | ((ulong)(data>>0) & (ulong)0x00ff00)>>8;
			tmp1 = ((ulong)(data>>16) & (ulong)0x00ff)<<8 | ((ulong)(data>>16) & (ulong)0x00ff00)>>8;
			tmp2 = ((ulong)(data>>32) & (ulong)0x00ff)<<8 | ((ulong)(data>>32) & (ulong)0x00ff00)>>8;
	...
	static public void Extract_headers(uint cnt, ulong tdata, ulong tuser)
	{
		switch(cnt)
		{
			...
			case 0U:
				...
				dst_port  = ((tuser>>16) & (ulong)0x00ff);
				break;
			case 1U:
