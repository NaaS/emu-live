...
import os.path, sys
print os.environ.get("PAX")
if not ("PAX" in os.environ):
    print "Please set the PAX env var to the pax directory"
    exit(1)
pax_ex_dir = os.path.abspath(os.path.expandvars('$PAX/examples'))
sys.path.insert(0, pax_ex_dir) # This is only temporary - disappears after python exits
...
class NatTopo(Topo):
    ...
    def __init__(self, n=2, **opts):
        ...
        nat = self.addNode("nat0", cls=PaxNode, ip="10.0.0.1", disable_arp=True)

        # Create the Outside host:
        # Use a hardcoded MAC address, because we provide the MAC to the NAT as next_hop
        publicHost = self.addHost("out0", mac="a2:b2:c3:d4:e5:f6", ip="10.0.0.4")
        ...
        for i in range(1, n+1):
            ...
            networkMac = "%0.2x:b2:c3:d4:e5:f6" % (16 * (i + 10) + 2) # Start at b2:b2:c3:d4:e5:f6 for in1
            intfMac = "%0.2x:22:33:44:55:66" % (16 * (i + 1) + 2) # Start at 22:22:33:44:55:66 for in1
            name = "in%d" % i
            print "<>> networkIP %s  networkMac %s  intfMac %s  name %s" % (networkIP, networkMac, intfMac, name)
            # Create the host with an IP on the inside network
            host = self.addHost(name, ip=networkIP, mac=networkMac)
            ...
            self.addLink(nat, host, intfName1="nat0-eth%d" % i, addr1=intfMac, params1={"ip":"192.168.1.1/24"})
...
def test():
    ...
    cmd = os.path.expandvars('$PAX/Bin/Pax.exe mn_test/nat_wiring.json bin/Release/Nat.dll')
    if config.X_windows:
